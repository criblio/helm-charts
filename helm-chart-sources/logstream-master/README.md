...
The cribl container's entrypoint.sh file looks for up to 30 environment variables that are assumed to be shell script snippets to be executed before LogStream startup (`CRIBL\_BEFORE\_START\_CMD\_[1-30]`). It also looks for up to 30 environment variables that are to be executed after LogStream startup (`CRIBL\_AFTER\_START\_CMD\_[1-30]`). The variables do need to be in order, and can not skip a number. (The `entrypoint.sh` script breaks the loop the first time it doesn't find an env var, so if you have `CRIBL\_BEFORE\_START\_CMD\_1` skipping to `CRIBL\_BEFORE\_START\_CMD\_3`, `CRIBL\_BEFORE\_START\_CMD\_3` will not be executed.)
